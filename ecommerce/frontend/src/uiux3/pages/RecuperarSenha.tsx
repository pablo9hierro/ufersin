...
export default function Uiux3RecuperarSenha() {
  ...
  return (
    <Shell>
      <div className="px-4 sm:px-8 pt-8 pb-16 max-w-sm mx-auto">
        <div className="u3-panel p-7">
          <div className="text-center mb-6">
            <p className="font-black text-lg">Ufersin</p>
            <p className="text-sm u3-dim mt-1.5 flex items-center justify-center gap-1.5">
          ...
          {step === 'whatsapp' && (
            <div className="space-y-3">
              <p className="text-sm u3-dim">Informe o WhatsApp da sua conta — vamos mandar um código de 3 dígitos por lá.</p>
              <div>
              ...
              <button onClick={sendCode} disabled={loading} className="u3-pill-primary w-full py-2.5 flex items-center justify-center gap-2 mt-1">
          ...
          {step === 'code' && (
            <div className="space-y-3">
              <p className="text-sm u3-dim">Digite o código de 3 dígitos que chegou no seu WhatsApp. Vale por 10 minutos.</p>
              <div>
              ...
              {error && <p className="text-xs text-red-500">{error}</p>}
              <button onClick={verifyCode} disabled={loading} className="u3-pill-primary w-full py-2.5 flex items-center justify-center gap-2 mt-1">
                {loading && <Loader2 className="w-4 h-4 animate-spin" />} Confirmar código
              </button>
              <button onClick={sendCode} disabled={loading} className="text-xs u3-dim block mx-auto">
                Reenviar código
              </button>
            </div>
          )}
          ...
          {step === 'password' && (
            <div className="space-y-3">
              <p className="text-sm u3-dim">Escolha sua nova senha de 4 dígitos.</p>
              <div>
                <label className="text-xs font-semibold u3-dim">Nova senha (4 dígitos)</label>
              ...
              <div>
                <label className="text-xs font-semibold u3-dim">Confirmar senha</label>
              ...
              {error && <p className="text-xs text-red-500">{error}</p>}
              <button onClick={resetPassword} disabled={loading} className="u3-pill-primary w-full py-2.5 flex items-center justify-center gap-2 mt-1">
          ...
          {step === 'done' && (
            <div className="space-y-4 text-center">
              ...
              <button onClick={() => navigate('/catalogo')} className="u3-pill-primary w-full py-2.5">
                Voltar ao início
              </button>
            </div>
          )}
