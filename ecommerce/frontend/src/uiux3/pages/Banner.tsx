...
// carrinho normal), só reapresentado com os componentes u3-*.
export default function Uiux3Banner() {
  ...
  return (
    <Shell>
      <div className="px-4 sm:px-8 pt-5 pb-32 max-w-xl mx-auto">
        ...
        <p className="text-xs u3-dim mb-5">
        ...
        <div className="space-y-2">
          {promotionProducts.map((product) => {
            ...
            return (
              <div key={product.id} className="u3-panel p-3 flex items-center gap-3">
                <button type="button" onClick={() => setInfoProduct(product)} className="w-14 h-14 rounded-xl flex items-center justify-center overflow-hidden shrink-0" style={{ background: 'var(--u3-surface-light)' }}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {pd ? (
                    <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
                      ...
                      <span className="text-xs font-semibold u3-accent">{discountText(pd.discount_type, pd.discount_value)}</span>
                      <span className="u3-accent text-sm font-bold">{currency(finalPrice(product.price, pd.discount_type, pd.discount_value))}</span>
                    </div>
                  ) : (
                    <p className="u3-accent text-sm font-bold mt-0.5">{currency(product.price)}</p>
                  )}
                </div>
                {isKit ? (
                  <span className="text-xs u3-dim shrink-0">Incluso no kit</span>
                ) : outOfStock ? (
                  <span className="text-xs font-semibold u3-dim shrink-0">Esgotado</span>
                ) : inCart > 0 ? (
                  <div className="flex items-center gap-1.5 u3-panel !rounded-lg px-2 py-1.5 shrink-0">
                    <button onClick={() => bannerCart.changeQty(product.id, -1)} className="w-6 h-6 flex items-center justify-center u3-accent">
                      <Minus className="w-3.5 h-3.5" />
                    </button>
                    <span className="text-sm w-4 text-center">{inCart}</span>
                    <button onClick={() => bannerCart.addItem(product.id, product.quantity)} disabled={inCart >= product.quantity} className="w-6 h-6 flex items-center justify-center u3-accent disabled:opacity-30">
                      <Plus className="w-3.5 h-3.5" />
                    </button>
                  </div>
                ) : (
                  <button onClick={() => bannerCart.addItem(product.id, product.quantity)} className="shrink-0 w-9 h-9 flex items-center justify-center u3-pill-primary !rounded-xl" aria-label={`Adicionar ${product.name}`}>
      ...
      <div className="u3-panel u3-panel-bottom-bar fixed bottom-0 left-0 right-0 z-30 !rounded-none !border-x-0 !border-b-0 px-4 sm:px-8 py-4">
        <div className="max-w-xl mx-auto">
          <div className="flex justify-between text-xs u3-dim mb-1">
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="u3-accent font-black text-lg">{currency(summary.total)}</span>
          </div>
          <button onClick={() => navigate('/banner/checkout')} disabled={bannerCart.items.length === 0} className="u3-pill-primary w-full text-base py-3.5 disabled:opacity-40">
            Finalizar Pedido
          </button>
        </div>
      </div>
      ...
      {infoProduct && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setInfoProduct(null)}>
          <div className="u3-panel p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">{infoProduct.name}</h3>
              <button onClick={() => setInfoProduct(null)} className="u3-dim" aria-label="Fechar">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {infoProduct.description && <p className="text-sm u3-dim mb-2">{infoProduct.description}</p>}
            <div className="flex justify-between text-sm">
              <span className="u3-dim">Preço</span>
              <span className="u3-accent font-bold">{currency(infoProduct.price)}</span>
            </div>
          </div>
        </div>
      )}
