...
// promotion_id sempre preenchido no pedido. Mesmo formulário simplificado
// (texto, sem mapa) do Checkout padrão deste estilo.
export default function Uiux3BannerCheckout() {
  ...
  const orderTotal = Math.max(subtotal - promotionProductDiscount, 0) + shippingPrice
  ...
  return (
    <Shell>
      <div className="px-4 sm:px-8 pt-5 pb-20 max-w-xl mx-auto">
        ...
        <p className="text-xs u3-dim mb-4">Este checkout é exclusivo pros itens escolhidos na promoção.</p>

        <div className="u3-panel p-3 mb-5 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs u3-accent font-medium">{promotion.promotion_type === 'kit' ? 'Pacote fechado' : 'Carrinho selfie service'}</p>
          </div>
        </div>
        ...
        <div className="u3-panel p-4 sm:p-6 space-y-5">
          ...
          {apenasRetirada ? (
            ...
            <PickupOnlyNotice config={tenantConfig} dimClass="u3-dim" />
            ...
            <>
              <div>
                ...
                {customer.lat != null && customer.lng != null ? (
                  ...
                  className="u3-panel w-full flex items-center gap-3 px-3.5 py-2.5 text-left">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-1">
              {(
                ...
                <button key={value} type="button" onClick={() => { setPaymentMethod(value); if (value !== 'dinheiro') setCashCents(0) }} className={paymentMethod === value ? 'u3-pill-primary flex flex-col items-center gap-1.5 py-3 text-sm' : 'u3-pill-secondary flex flex-col items-center gap-1.5 py-3 text-sm'}>
            ...
            {paymentMethod === 'dinheiro' && (
              <CashAmountInput
                ...
                trocoClassName={`text-sm font-semibold whitespace-nowrap ${cashCents / 100 < orderTotal ? 'text-amber-500' : 'u3-accent'}`}
              />
            )}
          </div>
            ...
            {appliedCoupon ? (
              <div className="flex items-center justify-between u3-panel px-3.5 py-2.5">
                <span className="flex items-center gap-2 text-sm font-medium">
                  <Tag className="w-4 h-4 u3-accent" /> {appliedCoupon.code}
                </span>
                <button
                  type="button"
                  onClick={() => {
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={applyCoupon} disabled={couponChecking || !couponInput.trim()} className="u3-pill-secondary px-4 shrink-0">
          ...
          <div className="u3-panel px-4 py-4 space-y-1.5 text-sm">
            ...
            {lines.map((l) => (
              <div key={l.product.id} className="flex justify-between text-xs u3-dim pl-3">
            ...
            <div className="flex justify-between u3-dim">
              <span>Frete</span>
              <span>{pickup ? 'Retirada no local' : customer.lat != null ? currency(shippingPrice) : 'Calculado ao escolher o endereço'}</span>
            </div>
            <div className="flex justify-between items-center pt-1.5">
              <span className="font-bold">Total</span>
              <span className="u3-accent font-black text-lg">{currency(Math.max(subtotal - promotionProductDiscount, 0) + shippingPrice)}</span>
            </div>
          </div>

          {error && <p className="text-xs text-red-500">{error}</p>}

          <button onClick={handleSubmit} disabled={submitting} className="u3-pill-primary w-full text-base py-3.5 flex items-center justify-center gap-2">
