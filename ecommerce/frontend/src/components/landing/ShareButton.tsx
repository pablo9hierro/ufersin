...
type LayoutVariant = 'u2' | 'u3' | 'u4' | 'sunset'

const TRIGGER_CLASS: Record<LayoutVariant, string> = {
  u2: 'u2-btn-secondary inline-flex items-center justify-center gap-2 px-4 py-2.5 mx-auto mt-1',
  u3: 'u3-pill-secondary inline-flex items-center justify-center gap-2 px-4 py-2.5 mx-auto mt-1',
  u4: 'u4-btn-secondary inline-flex items-center gap-2 px-4 py-2.5 w-fit mt-1',
  sunset: '',
}
...
const PANEL_CLASS: Record<LayoutVariant, string> = {
  ...
  u3: 'rounded-3xl p-5',
  u4: 'u4-panel p-5',
  sunset: 'rounded-2xl p-5 bg-son-black border border-white/10',
}
...
/** Botão inline das landings — painel com pilha de redes (só as preenchidas). */
export default function ShareButton({ links, variant }: { links: ShareLink[]; variant: LayoutVariant }) {
    ...
    <>
      ...
      {open && (
        <>
          ...
          <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 z-50 max-w-xs mx-auto">
            <div className={PANEL_CLASS[variant]} style={variant === 'u3' ? { background: 'var(--u3-surface)' } : undefined}>
              <div className="flex items-center justify-between gap-3 mb-4">
                <p className="font-bold text-sm">Redes sociais</p>
              ...
              <ul
                className={`share-stack share-stack-${variant} is-open`}
                style={{ minHeight: `${3.25 + (links.length - 1) * 3.5}rem` }}
              >
                {links.map((link, i) => (
                  <li
                    key={link.network}
                    className="share-stack-item"
