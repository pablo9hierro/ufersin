      ...
      {steps.map((step, i) => {
        ...
        return (
          <li key={step.label} className="flex-1 flex flex-col items-center text-center relative">
            {i > 0 && <span className="absolute top-3 right-1/2 w-full h-0.5" style={{ background: i <= currentIndex ? 'var(--uf-blue)' : 'var(--uf-surface-light)' }} />}
            <span
              className="relative z-10 w-6 h-6 rounded-full flex items-center justify-center text-white text-[10px] font-bold"
              style={{ background: done ? 'linear-gradient(135deg, var(--uf-blue), var(--uf-purple))' : 'var(--uf-surface-light)' }}
            >
              {done && i < currentIndex ? <Check className="w-3 h-3" /> : i + 1}
            </span>
            <span className={`text-[10px] mt-1.5 leading-tight ${done ? 'font-semibold' : 'u2-dim'}`}>{step.label}</span>
          </li>
        )
      })}
    </ol>
