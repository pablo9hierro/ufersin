...
export default function PromoCarousel() {
  ...
  return (
    <div className="px-4 sm:px-8 pt-5">
      <div
        className="u2-card relative overflow-hidden aspect-[2/1] max-w-xl mx-auto"
        ...
        <div className="flex items-center justify-center gap-1.5 mt-2.5">
          {items.map((it, i) => (
            <span
              ...
              style={{ width: i === activeIndex ? 16 : 6, background: i === activeIndex ? 'var(--uf-blue)' : 'color-mix(in srgb, var(--uf-silver-dim) 35%, transparent)' }}
            />
          ))}
        </div>
      )}
      {isDemoModeActive() && (
        <p className="text-xs text-center u2-dim mt-2">Banners de exemplo — crie os seus no painel.</p>
      )}
    </div>
  )
}
