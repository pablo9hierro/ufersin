...
export default function Uiux2Consultar() {
    ...
    <Shell>
      <div className="px-4 sm:px-8 pt-5 pb-16 max-w-xl mx-auto">
        ...
        <div className="flex gap-2 mb-6">
          <input className="u2-surface flex-1 px-3.5 py-2.5 text-sm outline-none" value={phone} onChange={(e) => setPhone(formatPhone(e.target.value))} placeholder="(83) 99999-9999" type="tel" inputMode="numeric" />
          <button onClick={() => search(phone)} disabled={loading} className="u2-btn-primary px-5">
        ...
        {orders === null ? null : orders.length === 0 ? (
          ...
          <ul className="space-y-4">
            {orders.map((order) => (
              <li key={order.id} className="u2-card p-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs u2-dim">Pedido #{order.id.slice(0, 8)}</span>
                  <span className="u2-badge px-2.5 py-1 text-[11px]">{STATUS_LABELS[order.status]}</span>
                </div>
                ...
                <div className="flex items-center justify-between text-sm mb-1">
                  <span className="u2-dim">{order.delivery_type === 'retirada' ? 'Retirada no local' : `Entrega em ${order.neighborhood ?? '-'}`}</span>
                  <span className="u2-accent font-bold">{currency(order.total)}</span>
                </div>
                ...
                {order.status === 'em_rota_de_entrega' && (
                  <>
                    ...
                    <div className="mt-3 u2-surface overflow-hidden">
