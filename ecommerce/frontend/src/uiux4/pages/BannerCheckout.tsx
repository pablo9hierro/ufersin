...
export default function Uiux4BannerCheckout() {
  ...
  return (
    <Shell>
      <div className="px-4 sm:px-8 pt-5 pb-20 max-w-xl mx-auto">
        ...
        <p className="text-xs u4-dim mb-4">Este checkout é exclusivo pros itens escolhidos na promoção.</p>

        <div className="u4-panel p-3 mb-5 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs u4-accent font-medium">{promotion.promotion_type === 'kit' ? 'Pacote fechado' : 'Carrinho selfie service'}</p>
          </div>
        </div>
        ...
        <div className="u4-panel p-4 sm:p-6 space-y-5">
            ...
            {appliedCoupon ? (
              <div className="flex items-center justify-between u4-input px-3.5 py-2.5">
                <span className="flex items-center gap-2 text-sm font-medium">
                  <Tag className="w-4 h-4 u4-accent" /> {appliedCoupon.code}
                </span>
                <button
                  type="button"
                  onClick={() => {
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={applyCoupon} disabled={couponChecking || !couponInput.trim()} className="u4-btn-secondary px-4 shrink-0">
          ...
          <div className="u4-input px-4 py-4 space-y-1.5 text-sm">
            ...
            {lines.map((l) => (
              <div key={l.product.id} className="flex justify-between text-xs u4-dim pl-3">
            ...
            <div className="flex justify-between items-center pt-1.5">
              ...
              <span className="u4-accent font-black text-lg">{currency(Math.max(subtotal - promotionProductDiscount, 0))}</span>
            </div>
          </div>

          {error && <p className="text-xs text-red-500">{error}</p>}

          <button onClick={handleSubmit} disabled={submitting} className="u4-btn-primary w-full text-base py-3.5 flex items-center justify-center gap-2">
