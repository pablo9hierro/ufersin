...
export default function Uiux4Historico() {
      ...
      <div className="px-4 sm:px-8 pt-5 pb-16 max-w-xl mx-auto">
        ...
        {loading ? (
          ...
          <ul className="space-y-4">
            {orders.map((order) => (
              <li key={order.id} className="u4-panel p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs u4-dim">Pedido #{order.id.slice(0, 8)}</span>
                  <span className="u4-chip px-2.5 py-1 text-[11px]">{STATUS_LABELS[order.status]}</span>
                </div>
                ...
                <div className="flex items-center justify-between text-sm mb-3">
                  <span className="u4-dim">{order.delivery_type === 'retirada' ? 'Retirada no local' : `Entrega em ${order.neighborhood ?? '-'}`}</span>
                  <span className="u4-accent font-bold">{currency(order.total)}</span>
                </div>
                <button onClick={() => handleBuyAgain(order)} disabled={buyingAgainId === order.id} className="u4-btn-secondary w-full flex items-center justify-center gap-2 text-sm py-2 disabled:opacity-60">
