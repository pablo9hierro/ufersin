...
export default function Uiux4Pagamento() {
  ...
  if (!order) {
    return (
      <Shell>
        <p className="text-center u4-dim py-20">Pedido não encontrado.</p>
      </Shell>
    )
  }
  ...
  return (
    <Shell>
      <div className="px-4 sm:px-8 pt-5 pb-20 max-w-sm mx-auto text-center">
        {paid ? (
          <div className="py-10">
            <PartyPopper className="w-14 h-14 u4-accent mx-auto mb-4" />
            <h1 className="text-xl font-black mb-2">Pagamento confirmado!</h1>
            <p className="u4-dim mb-8">Seu pedido já está sendo preparado. Você vai receber atualizações pelo WhatsApp.</p>
            <Link to={`/consultar?order=${order.id}`} className="u4-btn-primary inline-flex px-6 py-3">
              Acompanhar pedido
            </Link>
          </div>
          ...
          <>
            ...
            <p className="u4-dim text-sm mb-6">Escaneie o QR code ou copie o código abaixo.</p>
            ...
            <p className="u4-accent font-black text-2xl mb-6">{currency(order.total)}</p>

            {order.pix_copia_cola && (
              <button onClick={handleCopy} className="u4-btn-secondary w-full mb-4 text-sm py-3 flex items-center justify-center gap-2">
                {copied ? <Check className="w-4 h-4 shrink-0" /> : <Copy className="w-4 h-4 shrink-0" />}
                {copied ? 'Copiado!' : 'Copiar código Pix'}
              </button>
            )}

            <div className="flex items-center justify-center gap-2 text-xs u4-dim mb-6">
              <Loader2 className="w-3.5 h-3.5 animate-spin" />
              Aguardando confirmação do pagamento...
            </div>

            <button onClick={handleSimulate} className="text-xs u4-dim underline">
