...
// carrinho normal), só reapresentado com os componentes u4-*.
export default function Uiux4Banner() {
  ...
  return (
    <Shell>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {pd ? (
                    <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
                      ...
                      <span className="text-xs font-semibold u4-accent">{discountText(pd.discount_type, pd.discount_value)}</span>
                      <span className="u4-accent text-sm font-bold">{currency(finalPrice(product.price, pd.discount_type, pd.discount_value))}</span>
                    </div>
                  ) : (
                    <p className="u4-accent text-sm font-bold mt-0.5">{currency(product.price)}</p>
                  )}
                </div>
                {isKit ? (
                  <span className="text-xs u4-dim shrink-0">Incluso no kit</span>
                ) : outOfStock ? (
                  <span className="text-xs font-semibold u4-dim shrink-0">Esgotado</span>
                ) : inCart > 0 ? (
                  <div className="flex items-center gap-1.5 u4-input !rounded-lg px-2 py-1.5 shrink-0">
                    <button onClick={() => bannerCart.changeQty(product.id, -1)} className="w-6 h-6 flex items-center justify-center u4-accent">
                      <Minus className="w-3.5 h-3.5" />
                    </button>
                    <span className="text-sm w-4 text-center">{inCart}</span>
                    <button onClick={() => bannerCart.addItem(product.id, product.quantity)} disabled={inCart >= product.quantity} className="w-6 h-6 flex items-center justify-center u4-accent disabled:opacity-30">
                      <Plus className="w-3.5 h-3.5" />
                    </button>
                  </div>
                ) : (
                  <button onClick={() => bannerCart.addItem(product.id, product.quantity)} className="shrink-0 w-9 h-9 flex items-center justify-center u4-btn-primary !rounded-xl" aria-label={`Adicionar ${product.name}`}>
      ...
      <div className="u4-input u4-input-bottom-bar fixed bottom-0 left-0 right-0 z-30 !rounded-none !border-x-0 !border-b-0 px-4 sm:px-8 py-4">
        <div className="max-w-xl mx-auto">
          <div className="flex justify-between text-xs u4-dim mb-1">
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="u4-accent font-black text-lg">{currency(summary.total)}</span>
          </div>
          <button onClick={() => navigate('/banner/checkout')} disabled={bannerCart.items.length === 0} className="u4-btn-primary w-full text-base py-3.5 disabled:opacity-40">
            Finalizar Pedido
          </button>
        </div>
      </div>
      ...
      {infoProduct && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setInfoProduct(null)}>
          <div className="u4-panel p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">{infoProduct.name}</h3>
              <button onClick={() => setInfoProduct(null)} className="u4-dim" aria-label="Fechar">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="u4-accent font-bold">{currency(infoProduct.price)}</span>
            </div>
          </div>
        </div>
      )}
