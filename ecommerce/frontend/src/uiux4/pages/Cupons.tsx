...
export default function Uiux4Cupons() {
      ...
      <div className="px-4 sm:px-8 pt-5 pb-16 max-w-xl mx-auto">
        ...
        <div className="flex gap-2 mb-4">
          {(['ativos', 'inativos', 'historico'] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={tab === t ? 'u4-btn-primary flex-1 py-2 text-xs font-semibold capitalize' : 'u4-btn-secondary flex-1 py-2 text-xs font-semibold capitalize'}>
              {t}
            </button>
          ))}
        </div>

        <div className="u4-panel p-4 sm:p-6">
          {loading || !data ? (
        ...
        {revealOpen && (
          <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-sm flex flex-col items-center justify-center gap-6 p-6">
            {!preview ? <Loader2 className="w-8 h-8 animate-spin u4-accent" /> : <CouponTicket coupon={preview} />}

            {revealStage === 'revealed' && preview && (
              <button type="button" onClick={confirmClaim} className="u4-btn-primary px-8 py-3 text-sm">
                Resgatar cupom
              </button>
            )}
            {revealStage === 'claiming' && (
              <p className="flex items-center gap-2 text-sm u4-dim">
