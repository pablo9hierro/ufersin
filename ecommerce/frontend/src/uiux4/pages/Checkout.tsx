...
import { ChevronDown, CreditCard, Gift, Home, Loader2, MapPin, Package, QrCode, Tag, Wallet } from 'lucide-react'
    ...
    return (
      <Shell>
        <div className="px-4 sm:px-8 pt-16 flex justify-center">
          <Loader2 className="w-6 h-6 animate-spin u4-dim" aria-label="Carregando checkout" />
        </div>
      </Shell>
    )
  }
        ...
        <div className="u4-panel p-4 sm:p-6 space-y-5">
          {customerAuth.customer ? (
            <div className="flex items-center justify-between gap-3 text-sm">
              ...
              <button type="button" onClick={() => customerAuth.logout()} className="text-xs u4-dim underline shrink-0">
                Trocar conta
              </button>
            </div>
          ) : (
          ...
          <div>
              ...
              {([
                ...
                .map(({ value, label, icon: Icon }) => (
                  <button
                    ...
                    className={
                      paymentMethod === value
                        ? 'u4-btn-primary flex flex-col items-center gap-1.5 py-3 text-sm'
                        : 'u4-btn-secondary flex flex-col items-center gap-1.5 py-3 text-sm'
                    }
                  >
            ...
            {!pickup && !tenantConfig?.tem_motoboy_proprio && (
              <p className="text-xs u4-dim mt-1.5">Esta loja não tem motoboy próprio — entrega só é aceita com Pix pago antes. Outras formas de pagamento são só para retirada.</p>
            )}
            {!pickup && !!tenantConfig?.tem_motoboy_proprio && entregaSomentePix && (
              <p className="text-xs u4-dim mt-1.5">Entrega só com pagamento feito antes (Pix ou cartão) — dinheiro fica só pra retirada na loja.</p>
            )}
            {paymentMethod === 'dinheiro' && (
              <CashAmountInput
                ...
                trocoClassName={`text-sm font-semibold whitespace-nowrap ${cashCents / 100 < total ? 'text-amber-500' : 'u4-accent'}`}
              />
            )}
          </div>
