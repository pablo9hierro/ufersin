...
export default function EletronicaDiagnosticSection({
  ...
  if (loadingCatalog) {
    return (
      <div className="flex items-center gap-2 text-[#d4d4d8]/50 text-sm py-4">
  ...
  return (
    <section className="space-y-4">
      <h3 className="text-xs font-bold text-[#d4d4d8]/40 uppercase tracking-wider">Diagnóstico do Aparelho</h3>
      ...
      <div>
        <p className="text-xs font-semibold text-[#d4d4d8]/50 mb-2">Marca</p>
          ...
          {brands.map((b) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                selectedBrandId === b.id ? 'bg-[#e0211a] text-white border-[#e0211a]' : 'bg-white/5 border-white/10 text-[#d4d4d8]/70 hover:border-[#e0211a]/40'
              }`}
            >
              {b.name}
            </button>
      ...
      {selectedBrandId && modelList.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-[#d4d4d8]/50 mb-2">Modelo</p>
          <div className="flex flex-wrap gap-2">
            {modelList.map((m) => (
              <button
                key={m}
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                  selectedModelPill === m ? 'bg-[#e0211a] text-white border-[#e0211a]' : 'bg-white/5 border-white/10 text-[#d4d4d8]/70 hover:border-[#e0211a]/40'
                }`}
              >
                {m}
              </button>
      ...
      {serviceCards.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-[#d4d4d8]/50 mb-2">
            Serviços
            {selectedServices.length > 0 && (
            ...
            {serviceCards.map((s) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 w-40 rounded-xl p-3 text-left border-2 transition-all ${sel ? 'border-[#e0211a] bg-red-500/10' : 'border-white/10 bg-white/5 hover:border-[#e0211a]/40'}`}
                >
                  <div className="text-xs font-semibold text-white leading-tight">{s.repair_type}</div>
                  {s.description && <div className="text-[10px] text-[#d4d4d8]/40 mt-1 leading-snug line-clamp-2">{s.description}</div>}
                  <div className="text-[#e0211a] font-bold text-xs mt-2">R$ {money(s.price)}</div>
        ...
        <div className="bg-white/5 rounded-xl p-3 space-y-1.5">
          {selectedServices.map((s) => (
            <div key={s.id} className="flex justify-between text-sm">
              <span className="text-[#d4d4d8]/80">{s.repair_type}</span>
              <span className="font-semibold text-white">R$ {money(s.price)}</span>
            </div>
          ))}
          <div className="border-t border-white/10 pt-1.5 flex justify-between font-bold text-sm text-white">
            <span>Total auto</span>
            <span className="text-[#e0211a]">R$ {money(autoTotal)}</span>
          </div>
        </div>
      )}
      ...
      <div>
        <label className="block text-xs text-[#d4d4d8]/50 mb-1">
          Valor ajustado do orçamento (R$) <span className="font-normal text-[#d4d4d8]/40">— opcional</span>
        </label>
        ...
        {customTotal && <p className="text-xs text-amber-300 mt-1">Orçamento ajustado: R$ {money(finalTotal)}</p>}
      </div>

      <div>
        <label className="block text-xs text-[#d4d4d8]/50 mb-1">Observações técnicas</label>
        <textarea
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
          placeholder="Ex: Tela com manchas internas, conector oxidado, bateria inchada..."
          rows={3}
          className="w-full rounded-xl bg-white/5 border border-white/10 text-white px-3 py-2.5 text-sm outline-none focus:border-[#e0211a] resize-none"
        />
      </div>

      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs text-[#d4d4d8]/50">Fotos/vídeos do aparelho</label>
        ...
        {mediaUrls.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {mediaUrls.map((url) => (
              <div key={url} className="relative w-14 h-14 rounded-lg border border-white/10 overflow-hidden bg-white/5">
      ...
      {error && (
        <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-xl p-3 text-red-300 text-xs">
      ...
      <div className="space-y-2">
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 text-sm font-medium text-[#d4d4d8]/70 border border-white/10 rounded-xl py-2 hover:bg-white/5 transition-colors disabled:opacity-50"
        >
        ...
        {(pdfUrl || selectedServices.length > 0) && (
          <div className="grid grid-cols-2 gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-center gap-1.5 text-sm font-medium text-[#d4d4d8]/70 border border-white/10 rounded-xl py-2 hover:bg-white/5 transition-colors"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-center gap-1.5 text-sm font-medium text-emerald-300 border border-emerald-500/20 rounded-xl py-2 hover:bg-emerald-500/10 transition-colors"
            >
