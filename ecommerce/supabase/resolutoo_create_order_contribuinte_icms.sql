-- Etapa 2 do projeto de perfis fiscais (fiscal-part-2): campo
-- `contribuinte_icms` do contexto de venda, espelhando a coluna real
-- `orders.destinatario_contribuinte_icms` adicionada no motor Railway
-- (ecommerce/backend/migrations/0065_orders_destinatario_contribuinte_icms.sql).
-- Só relevante pra CNPJ; default `false` preserva 100% o comportamento
-- atual (não-contribuinte/consumidor final) pra quem não marca o novo
-- checkbox no checkout.

ALTER TABLE loja.orders ADD COLUMN IF NOT EXISTS destinatario_contribuinte_icms boolean NOT NULL DEFAULT false;
...
CREATE OR REPLACE FUNCTION resolutoo.create_order(
  ...
  p_contribuinte_icms boolean DEFAULT false
)
...
BEGIN
  ...
  INSERT INTO loja.orders (
    ...
    destinatario_contribuinte_icms
  ) VALUES (
    ...
    coalesce(p_contribuinte_icms, false)
  );
