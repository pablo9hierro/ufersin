...
pub fn build() -> OpenApi {
    ...
    let routes: Vec<(&str, HttpMethod, &str, &str, bool, bool)> = vec![
        ...
        ("/api/onboarding/fiscal", Get, "Ler dados fiscais da empresa já cadastrados", "Fiscal", true, false),
        ("/api/onboarding/fiscal", Post, "Salvar dados fiscais da empresa (CNPJ, endereço, regime) e criar/atualizar no Jubilados", "Fiscal", true, true),
        ("/api/onboarding/fiscal/certificado", Post, "Upload do certificado digital A1 (.pfx/.p12, multipart) -- validado e repassado ao Jubilados, nunca persistido aqui", "Fiscal", true, true),
        ("/api/onboarding/fiscal/certificado/status", Get, "Status do certificado digital salvo no Jubilados (válido/vencido), sem expor o certificado", "Fiscal", true, false),
        ("/api/onboarding/cfops", Get, "CFOPs cadastrados pela empresa na plataforma", "Fiscal", true, false),
        ("/api/onboarding/cfops", Post, "Adicionar CFOP da tabela oficial ao cadastro da empresa", "Fiscal", true, true),
        ("/api/onboarding/cfops/search", Get, "Buscar CFOP na tabela oficial por código ou descrição", "Fiscal", true, false),
        ("/api/onboarding/cfops/default", Post, "Definir qual CFOP cadastrado é o padrão", "Fiscal", true, true),
        ("/api/onboarding/cfops/remove", Post, "Remover CFOP do cadastro da empresa", "Fiscal", true, true),
        ("/api/onboarding/fiscal-defaults", Get, "Ler CST/CSOSN/CEST/Classificação Tributária padrão da empresa", "Fiscal", true, false),
        ("/api/onboarding/fiscal-defaults", Post, "Salvar CST/CSOSN/CEST/Classificação Tributária padrão da empresa", "Fiscal", true, true),
        ("/api/onboarding/classificacao-tributaria", Get, "Tabela oficial de Classificação Tributária (IBS/CBS), passthrough do Jubilados", "Fiscal", true, false),
        ("/api/fiscal/states", Get, "Listar as 27 UFs", "Fiscal", false, false),
        ("/api/fiscal/states/{uf}/cities", Get, "Listar municípios do IBGE de uma UF", "Fiscal", false, false),
