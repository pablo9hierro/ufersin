/** Aplica landing_texts + atualiza get_public_tenant_config. */
import pg from 'pg'
...
const sqlPath = path.join(__dirname, '../migrations/0024_landing_texts.sql')
...
try {
  ...
  console.log('RPC ok: landing_texts=', r.rows[0]?.j?.landing_texts ?? null)
} finally {
  await client.end()
}
